...
if [[ "$@" =~ "beta"* ]]; then
	...
	# Be careful if something changes LOCALVERSION line
        sed -i "50s/.*/CONFIG_LOCALVERSION=\"-Alcor-HMP-${RELEASE_VERSION}-rc${DRONE_BUILD_NUMBER}-${RELEASE_CODENAME}\"/g" arch/arm64/configs/alcor_defconfig
	export INC="$(echo ${RC} | grep -o -E '[0-9]+')"
	INC="$((INC + 1))"
	sed -i "2s/.*/rc$INC/g" CURRENTVERSION
else
	export TYPE=stable
	export VERSION="Alcor-HMP-Stable-${RELEASE_VERSION}-${RELEASE_CODENAME}"
        # Be careful if something changes LOCALVERSION line
        sed -i "50s/.*/CONFIG_LOCALVERSION=\"-Alcor-HMP-${RELEASE_VERSION}-${RELEASE_CODENAME}\"/g" arch/arm64/configs/alcor_defconfig
fi
